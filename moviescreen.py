...
class MovieScreen(screen.Screen):
    ...
    def options(self):
        self.optionsframe = tk.Frame(self.frame, background='#81868F', padx=1, pady=1)
        self.optionsbackground = tk.Frame(self.optionsframe, background='#FFFFFF')
        self.optionsframe.grid(row=0, column=0, sticky="NW")
        self.optionsbackground.grid(row=0, column=0)

        self.wavelength = tk.StringVar(self.frame)
        self.wavelength.set("")
        ...
        self.cutoutlabel.grid(row=4, column=0, padx=20, pady=(0, 10), sticky="NW")

        self.cutoutslider = ttk.Scale(self.optionsbackground, from_=50, to=1000, orient=tk.HORIZONTAL, value=250, command=self.update_cutout)
        self.cutoutslider.grid(row=5, column=0, padx=20, pady=(0, 20), sticky="NW")
        ...
        self.movieframe.grid(row=0, column=1, padx=20, pady=20, sticky="NW")
        ...
        self.imagecanvas.get_tk_widget().grid(row=0, column=0)
    ...
    def play(self):
        self.playing = True
        self.imageslider = ttk.Scale(self.movieframe, from_=0, to=len(self.flare.images[int(self.wavelength.get())]), orient=tk.HORIZONTAL,
                                     length = 1000)
    ...
    def cycle(self):
        for i in range(len(self.cutouts)):
            self.full_images[i].get_tk_widget().grid(row=0, column=0)
            self.cutouts[i].get_tk_widget().grid(row=0, column=1)
            self.frame.update_idletasks()
            self.full_images[i].get_tk_widget().grid_forget()
            self.cutouts[i].get_tk_widget().grid_forget()

        self.idx = -1
        while self.active and self.playing:
            self.update_image((self.idx + 1)%len(self.full_images))
            time.sleep(self.speedslider.get()/10)

    def update_image(self, i, last=None):
        self.full_images[self.idx].get_tk_widget().grid_forget()
        self.full_images[i].get_tk_widget().grid(row=0, column=0)
